...
*   **Strict Security Posture:**
    *   **CSRF Protection:** Generates a secure, random `state` parameter before redirecting to Okta and strictly verifies it upon the callback.
    ...
    *   **Strict JWT Signature Validation:** Instead of blindly trusting the token, the application dynamically fetches the Okta organization's JSON Web Key Set (JWKS) and cryptographically verifies the RS256 signature, the issuer URL, and the intended audience (Client ID).
...
1.  **`auth.py` (The Security Layer):** 
    ...
    *   Exchanges authorization codes for identity tokens securely via backend HTTP requests (`httpx`).
    *   Performs rigorous cryptographic validation of the JWT using the `python-jose` library and Okta's public JWKS.
...
2.  **`app.py` (The Presentation Layer):** 
    ...
    *   Maintains secure session state `st.session_state` to store user identity and cryptographic validations.
